...
inherit toolchain-funcs flag-o-matic qmake-utils
...
src_prepare() {
	...
	local qt_ver=$(/usr/lib/qt5/bin/qmake -query QT_VERSION)
	sed -i "s/QT_TDESKTOP_VERSION_DEFAULT = 5.6.0/QT_TDESKTOP_VERSION_DEFAULT = ${qt_ver}/g" "${S}/Telegram/qt_static.pri" || die
	...
	sedargs=(
		...
		-e 's|(.*ps_unity_*)|// \1|'
		-e 's|(.*UnityLauncher*)|// \1|'
		-e 's|(.*unity_launcher*)|// \1|'
		-e 's|(.*lib_unity*)|// \1|'
	)
	sed -i -r "${sedargs[@]}" "${S}/Telegram/SourceFiles/pspecific_linux.cpp" || die "Can't nuke-unity patch 0"
	sed -i -r "${sedargs[@]}" "${S}/Telegram/SourceFiles/platform/linux/linux_libs.h" || die "Can't nuke-unity patch 1"
	sed -i -r "${sedargs[@]}" "${S}/Telegram/SourceFiles/platform/linux/linux_libs.cpp" || die "Can't nuke-unity patch 2"
	sed -i -r "${sedargs[@]}" "${S}/Telegram/SourceFiles/platform/linux/main_window_linux.cpp" || die "Can't nuke-unity patch 3"
...
src_compile() {
	local qt_ver=$(/usr/lib/qt5/bin/qmake -query QT_VERSION)
	export _qtver=$qt_ver
	export QT_TDESKTOP_VERSION="$qt_ver"
	export QT_TDESKTOP_PATH="/usr/lib/qt5/"
