...
inherit eutils flag-o-matic git-r3 subversion multibuild multilib \
...
pkg_pretend() {
	...
	if [[ ${MERGE_TYPE} != binary ]]; then
		...
		if ! $(tc-getCXX) -std=c++11 -o /dev/null "${T}"/test.cxx; then
			eerror "LLVM-${MY_PV} requires C++11-capable C++ compiler. Your current compiler"
...
src_unpack() {
	...
	if use clang; then
		ESVN_PROJECT="cfe"
		subversion_fetch http://llvm.org/svn/llvm-project/${ESVN_PROJECT}/branches/release_${MY_PV/./} tools/clang
		ESVN_PROJECT="clang-tools-extra"
		subversion_fetch http://llvm.org/svn/llvm-project/${ESVN_PROJECT}/branches/release_${MY_PV/./} tools/clang/tools/extra
		ESVN_PROJECT="compiler-rt"
		subversion_fetch http://llvm.org/svn/llvm-project/${ESVN_PROJECT}/branches/release_${MY_PV/./} projects/compiler-rt
	fi
ESVN_PROJECT="llvm"
}
...
multilib_src_install() {
	...
	if use clang; then
		...
		if ! multilib_is_native_abi; then
			...
			for i in "${clang_tools[@]}"; do
				rm "${ED%/}/usr/bin/${i}-${MY_PV}" || die
				cat > "${T}"/wrapper.tmp <<-_EOF_
					#!${EPREFIX}/bin/sh
					exec "${i}-${MY_PV}" $(get_abi_CFLAGS) "\${@}"
				_EOF_
				newbin "${T}"/wrapper.tmp "${i}-${MY_PV}"
			done
		fi
	fi
	...
	local lib= f= odylib= libpv=${MY_PV}
