...
if $WITH_INET; then
  ...
  #Allow my prefix
  $IP6TABLES -A INPUT -i $LAN_IFACE_INT -s $INET_PREFIX -j ACCEPT
  $IP6TABLES -A INPUT -i $LO_IFACE -s $INET_PREFIX -j ACCEPT
  $IP6TABLES -A INPUT -i $LAN_IFACE_EXT -s $INET_PREFIX -j ACCEPT
  $IP6TABLES -A INPUT -i $INET_IFACE -s $INET_PREFIX -j ACCEPT
fi
...
if $WITH_INET; then
  #Allow my prefix
  $IP6TABLES -A FORWARD -i $LAN_IFACE_INT -s $INET_PREFIX -j ACCEPT
  $IP6TABLES -A FORWARD -i $LO_IFACE -s $INET_PREFIX -j ACCEPT
  $IP6TABLES -A FORWARD -i $LAN_IFACE_EXT -s $INET_PREFIX -j ACCEPT
  $IP6TABLES -A FORWARD -i $INET_IFACE -s $INET_PREFIX -j ACCEPT

  $IP6TABLES -A FORWARD -o $LAN_IFACE_INT -d $INET_PREFIX -j ACCEPT
  $IP6TABLES -A FORWARD -o $LO_IFACE -d $INET_PREFIX -j ACCEPT
  $IP6TABLES -A FORWARD -o $LAN_IFACE_EXT -d $INET_PREFIX -j ACCEPT
  $IP6TABLES -A FORWARD -o $INET_IFACE -d $INET_PREFIX -j ACCEPT

  $IP6TABLES -A FORWARD -i $INET_IFACE -o $INET_IFACE -s $INET_PREFIX -j ACCEPT
  $IP6TABLES -A FORWARD -i $INET_IFACE -o $INET_IFACE -d $INET_PREFIX -j ACCEPT
  $IP6TABLES -A FORWARD -s $INET_PREFIX -d $INET_PREFIX -j ACCEPT
  $IP6TABLES -A FORWARD -s $INET_PREFIX -j ACCEPT
  $IP6TABLES -A FORWARD -d $INET_PREFIX -j ACCEPT
fi
...
if $WITH_INET; then
  ...
  #Allow my prefix
#   $IP6TABLES -A OUTPUT -o $LAN_IFACE_INT -s $INET_PREFIX -j ACCEPT
#   $IP6TABLES -A OUTPUT -o $LO_IFACE -s $INET_PREFIX -j ACCEPT
  $IP6TABLES -A OUTPUT -o $LAN_IFACE_EXT -s $INET_PREFIX -j ACCEPT
  $IP6TABLES -A OUTPUT -o $INET_IFACE -s $INET_PREFIX -j ACCEPT
fi
