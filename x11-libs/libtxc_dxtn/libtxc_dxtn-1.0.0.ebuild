...
HOMEPAGE="http://people.freedesktop.org/~mareko/libtxc_dxtn/"
#SRC_URI="http://cgit.freedesktop.org/~mareko/libtxc_dxtn/snapshot/${P}.tar.gz"
SRC_URI="http://cgit.freedesktop.org/~mareko/libtxc_dxtn/snapshot/${P}.tar.gz"
RESTRICT="mirror"
LICENSE="BSD"
SLOT="0"
...
src_compile() {
	...
	for ABI in `get_all_abis` ; do
		...
		CC=$(tc-getCC)
		emake -C "${ABI}/${P}" || die "emake (ABI=${ABI})"
	done
}
...
src_install() {
	local ABI
	for ABI in `get_all_abis` ; do
		dolib.so "${ABI}/${P}/libtxc_dxtn.so" || die "dolib.so libtxc_dxtn.so (ABI=${ABI})"
	done
}
